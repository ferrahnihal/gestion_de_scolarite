...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'ajouter_enseignant') {
    ...
    mysqli_query($conn, "INSERT INTO enseignant (Nom, Prenom, email, mot_de_passe, Module_id) VALUES ('$nom','$prenom','$email','$mdp','$mod_id')");
  }

  if ($action === 'supprimer_enseignant') {
    $id = (int)$_POST['id_Enseinant'];
    mysqli_query($conn, "DELETE FROM enseignant WHERE id=$id_Enseinant");
  }
  ...
  if ($action === 'modifier_enseignant') {
    ...
    $email  = mysqli_real_escape_string($conn, $_POST['email']);
    $mod_id = mysqli_real_escape_string($conn, $_POST['module_id']);
    mysqli_query($conn, "UPDATE enseignant SET Nom='$nom', Prenom='$prenom', email='$email', Module_id='$mod_id' WHERE id=$id");
  }
...
// Fonction statut en ligne (connecté dans les 15 dernières minutes)
function statutEnLigne($derniere_connexion) {
  if (!$derniere_connexion) return false;
  return (time() - strtotime($derniere_connexion)) / 60 <= 15;
}
...
<div class="wrap">
  ...
  >
            ...
            <table id="tableEtudiants">
              <thead>
                <tr><th>Matricule</th><th>Nom</th><th>Prénom</th><th>Niveau</th><th>Email</th><th>Statut</th><th>Actions</th></tr>
              </thead>
              <tbody>
                <?php
                $res = mysqli_query($conn, "SELECT * FROM etudiant");
                while($row = mysqli_fetch_assoc($res)):
                  $enLigne = statutEnLigne($row['derniere_connexion'] ?? null);
                ?>
                <tr>
                  ...
                  <td>
                    <span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:<?= $enLigne ? '#4CAF50' : '#bbb' ?>;margin-right:5px;"></span>
...
'<?= htmlspecialchars($row['Module_id'], ENT_QUOTES) ?>'
)" class="btn-action">✏️</button>
<button onclick="confirmerSupprimerEnseignant('<?= $row['id_Enseignant'] ?>')" class="btn-danger">🗑️</button>
                  </td>
                </tr>
                <?php endwhile; ?>
              </tbody>
      ...
      <div class="page" id="page-notes">
        <div class="card">
          ...
          <div id="nt-liste">
            <table>
              <thead><tr><th>Matricule</th><th>Module</th><th>Note</th><th>Type</th><th>Actions</th></tr></thead>
              <tbody>
                <?php
                $res_nt = mysqli_query($conn, "SELECT * FROM note");
                while($row = mysqli_fetch_assoc($res_nt)):
                ?>
                <tr>
                  <td><?= htmlspecialchars($row['Matricule']) ?></td>
                  <td><?= htmlspecialchars($row['ID_Module']) ?></td>
                  <td><?= htmlspecialchars($row['note']) ?></td>
                  <td><?= htmlspecialchars($row['Type_evaluation']) ?></td>
                  <td>
                    <button onclick="remplirModifierNote(<?= $row['ID_Module'] ?>,'<?= addslashes($row['note']) ?>','<?= addslashes($row['Type_evaluation']) ?>')" class="btn-action">✏️</button>
                    <button onclick="confirmerSupprimerNote(<?= $row['ID_Module'] ?>)" class="btn-danger">🗑️</button>
                  </td>
                </tr>
                <?php endwhile; ?>
              </tbody>
